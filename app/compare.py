...
from .extract import FIELDS,STAGES,clean,numeric
...
def compare_cells(cells):
    ...
    if any(c.get('status') in ['needs_review','conflict'] for c in cells):status='review'
...
def rows_for(docs):
    docs=sorted(docs,key=lambda d:(STAGES.index(d['stage']) if d['stage'] in STAGES else 3,d['fields']['印发日期'].get('value') or '',d['filename']))
    rows=[]
    for name in FIELDS:
        cs=[d['fields'][name] for d in docs]
        rows.append({'name':name,'kind':'fixed','cells':cs,'status':compare_cells(cs),'note':''})
    ...
    for name in names:
        ...
        if all(ns) and status not in ['review','missing']:
            keys=[(n['number'],n['upper'],n['unit'],n['qualifier']) for n in ns]
            if len(set(keys))==1 and len({c['value'] for c in cs})>1:status='equivalent'
            if len({n['unit'] for n in ns})==1 and not any(n['upper'] for n in ns):
                notes=[]
                for i in range(1,len(ns)):
                    a,b=Decimal(ns[i-1]['number']),Decimal(ns[i]['number'])
                    if a!=b:
                        note_i=f'{docs[i-1]["stage"]}→{docs[i]["stage"]}：{b-a:+f}{ns[i]["unit"]}'
                        if a:note_i+=f'（{(b-a)/a*100:+.2f}%）'
...
def export_xlsx(documents):
    ...
    for gi,ds in enumerate(groups.values(),1):
        ...
        for kind,label in [('fixed','固定字段'),('metric','建设指标')]:
            ...
            ws.append([docs[0]['project_name']]);ws.append(['字段']+[d['stage']+' | '+d['filename'] for d in docs]+['比较结果','说明'])
            for r in rows:
        ...
        for d in docs:
            for k,c in list(d['fields'].items())+[(m['name'],m) for m in d['metrics']]:
                ...
                if c.get('status') in ['needs_review','conflict']:
